...
class TwoLayerNet(torch.nn.Module):
    def __init__(self,D_in,H1):
        ...
        torch.nn.init.uniform_(self.control1.weight,0,0)
        torch.nn.init.uniform_(self.control1.bias,0,0)
        torch.nn.init.uniform_(self.control2.weight,0,0)
        torch.nn.init.uniform_(self.control2.bias,0,0)
...
for i in range(len(ref)-1):
    for j in range(n_sample):
        ...
        theta = np.random.uniform(ref[i][2]-dtheta,ref[i][2]+np.pi/2)
...
data = data.to(device)
# optimizer1 = torch.optim.Adam(model.parameters(), lr=1e-2, weight_decay=1e-5)
# optimizer2 = torch.optim.Adam(model.parameters(), lr=1e-2, weight_decay=1e-5)
# optimizer3 = torch.optim.Adam(model.parameters(), lr=1e-3, weight_decay=1e-5)
...
for j in range(0,1):
    for i in range(0,1000):
        ...
        beta = torch.atan(Lr/(Lr+Lf)*torch.sin(delta)/torch.cos(delta))
        new_x_tensor = x_tensor+0.01*vr*torch.cos(theta_tensor+beta)
        new_y_tensor = y_tensor+0.01*vr*torch.sin(theta_tensor+beta)
        new_theta_tensor = theta_tensor+0.01*vr/Lr*torch.sin(beta)

        error_x = (new_x_tensor-ref_x_tensor)**2
        error_y = (new_y_tensor-ref_y_tensor)**2
        error_pos = (new_x_tensor-ref_x_tensor)**2+(new_y_tensor-ref_y_tensor)**2
        error_goal = new_x_tensor**2+new_y_tensor**2
        error_y_cmp = torch.relu((new_y_tensor-ref_y_tensor)**2 - data[:,1]**2)
        # error_x_cmp = 
        ...
        error_cons = torch.relu(delta - np.pi/4)+torch.relu(-np.pi/4-delta)+torch.relu(torch.sign(data[:,1]*delta))

        error = error_cons.sum()+error_x.mean()
        ...
        # loss1 = error_pos.mean()
        # loss2 = error_theta.mean()
        # loss3 = error_y_cmp.mean()
        loss = error
        # if i%10 == 0:
        print(i,loss.item(),error_pos.mean().item(),error_cons.mean().item(),error_y_cmp.mean().item())
...
model = model.to(device)
x_init = np.random.uniform(-15-0.1,-15+0.1)
y_init = np.random.uniform(0-0.1,0+0.1)
...
for i in range(len(ref)-1):
    ...
    print(i,vr,delta,error_x,error_y,error_theta,trajectory[i][3])
