...
y_init = y_init = np.random.uniform(-1,1)
theta_init = 0
first_run = True
ref_tmp = ref
error_x_list = []
...
for i in range(4):
    if first_run:
        ...
        x_transform = np.random.uniform(-1,1)
        y_transform = np.random.uniform(-1,1)
        
        x_init = x_init + x_transform
        y_init = y_init + y_transform
...
plt.figure(1)
plt.xlabel('x posision')
plt.ylabel('y position')
plt.title('Vehicle Trjectory After Translation')
plt.show()
