...
for i in range(30):
    x_init_origin = np.random.uniform(-1,1)
    y_init_origin = np.random.uniform(-6,-4)
    theta_init_origin = np.random.uniform(0,np.pi)
    print("initial",x_init,y_init,theta_init*180/np.pi)

    theta_transform = np.random.uniform(0,2*np.pi)

    tmp1 = np.cos(theta_transform)*x_init_origin + np.sin(theta_transform)*y_init_origin
    tmp2 = -np.sin(theta_transform)*x_init_origin + np.cos(theta_transform)*y_init_origin
    x_init = tmp1
    y_init = tmp2
    theta_init = theta_init_origin - theta_transform

    for i in range(len(ref_origin)):
        tmp1 = np.cos(theta_transform)*ref_origin[i][0]+np.sin(theta_transform)*ref_origin[i][1]
        tmp2 = -np.sin(theta_transform)*ref_origin[i][0]+np.cos(theta_transform)*ref_origin[i][1]
        ref[i][0] = tmp1
        ref[i][1] = tmp2
        ref[i][2] = ref_origin[i][2] - theta_transform
