...
# x_init_fix = 0
# y_init_fix = -5
theta_init_fix = np.random.uniform(0,np.pi)
first_run = True
...
for i in range(1):
    if first_run:
        ...
        theta_transform = np.random.uniform(-np.pi, np.pi)
    ...
    # Transform initial position and reference to origin
    x_transform = 0 - ref[-1][0]
    y_transform = 0 - ref[-1][1]
    theta_transform = np.pi/2 - np.arctan2(ref[-1][1]-ref[0][1],ref[-1][0]-ref[0][0])
    ...
    for i in range(len(ref)-1):
        ...
        # val[0] = trajectory[i][1] + 0.01*vr*np.cos(trajectory[i][3])
        # val[1] = trajectory[i][2] + 0.01*vr*np.sin(trajectory[i][3])
        # val[2] = trajectory[i][3] + 0.01*vr/4*np.tan(delta)
        # tmp = vr/Lr * np.sin(beta)
        ...
        time.append(r.t)
        print(i,vr,delta,error_pos,val[2])
...
plt.title('Vehicle Trajectory After Rotation')
plt.xlim((-5.5,5.5))
plt.ylim((-5.5,5.5))
plt.show()
