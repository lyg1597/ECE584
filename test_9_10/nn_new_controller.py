...
for j in range(0,1):
    for i in range(0,10000):
        ...
        delta = torch.clamp(control_tensor[:,1],-np.pi*2/5,np.pi*2/5)
        ...
        new_x_tensor = x_tensor+0.01*vr*torch.cos(theta_tensor)
        new_y_tensor = y_tensor+0.01*vr*torch.sin(theta_tensor)
        new_theta_tensor = theta_tensor+0.01*delta
