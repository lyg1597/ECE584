...
def func1(t, vars, u):
    ...
    # dvx = dvx * math.cos(sc) - dvy * math.sin(sc)
    # dvy = dvx * math.sin(sc) + dvy * math.cos(sc)
...
def runModel(initalCondition, time_bound, time_step, ref_input):
    ...
    while t <= time_bound:
        ...
        ############## transforming the controller input for symmetric controller ##################

        # dot = ex
        # det = ey
        # sc = ref_input[3] # math.atan2(dot, det)

        # ex = ex * math.cos(sc) - ey * math.sin(sc)
        # ey = ex * math.sin(sc) + ey * math.cos(sc)
        # evx = evx * math.cos(sc) - evy * math.sin(sc)
        # evy = evx * math.sin(sc) + evy * math.cos(sc)
...
def TC_Simulate(waypoint: Waypoint, time_step: float,
    ...
    if mode == 'follow_waypoint':
        ...
        trace = runModel(mode_parameters[0:3] + list(initial_point), time_bound, time_step, [ref_vx, ref_vy, ref_vz,
                                                                                             sym_rot_angle,
                                                                                             sym_rot_angle_yz])
...
if __name__ == "__main__":
    ...
    init_y = np.random.uniform(-0.5,0.5)
    init_z = 0
    ...
    for i in range(4):
        trans_theta = 0
        if i != 0:
            trans_theta = np.random.uniform(0,np.pi*2)
