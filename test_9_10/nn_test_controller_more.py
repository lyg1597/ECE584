...
for i in range(30):
    ...
    delta = 0
    fct = 10
    for i in range(fct*(len(ref)-1)):
        j = int(i/fct)
        error_x = ref[j+1][0]-trajectory[i][1]
        error_y = ref[j+1][1]-trajectory[i][2]
        error_theta = (ref[j+1][2]-trajectory[i][3])%(np.pi*2)
        error_theta_cos = np.cos(error_theta)
        error_theta_sin = np.sin(error_theta)
        x_start.append(trajectory[i][1]-ref[j+1][0])
        y_start.append(trajectory[i][2]-ref[j+1][1])
        
        # error_y = np.abs(ref[j+1][1]-trajectory[i][2])
        # error_theta = np.cos(ref[j+1][2])-np.cos(trajectory[i][3])    
        symmetry_y = trajectory[i][2]-ref[j+1][1]
        ...
        x_end.append(val[0]-ref[j+1][0])
        y_end.append(val[1]-ref[j+1][1])
        trajectory.append([r.t,val[0],val[1],val[2]])
        error_pos = np.sqrt((val[0]-ref[j+1][0])**2+(val[1]-ref[j+1][1])**2)
